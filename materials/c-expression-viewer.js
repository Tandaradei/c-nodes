...
function plot(data) {
                           svg.each(function(orientation) {
                                    ...
                                    svg.selectAll('g').data([]).join();
                                    svg.selectAll('circle').data([]).join();
                                    svg.selectAll('text').data([]).join();
                                    svg.selectAll('path').data([]).join();
                                    ...
                                    .append("g")
                                    node.append("circle")
                                    .attr("class", function (d) {console.log(d.data.name); return "node " + d.data.type;})
                                    .attr("r", 10)
                                    .attr("cx", o.x)
                                    .attr("cy", o.y);
